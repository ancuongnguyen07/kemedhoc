...
let lemma_concat_info_get_length_fixed_length
  (i:info)
  : Lemma (ensures (
    concat_info_get_length i = (FBytes.repr_bytes i.label) + (length i.context) + (FBytes.repr_bytes i.okm_len)
  ))
  [SMTPat (concat_info_get_length i)]
  = ()
...
val construct_info_to_bytes:
  ...
  -> okm_len:okm_len_type
  -> Tot (lbytes (
    concat_info_get_length (construct_info label context okm_len)
  ))

val lemma_construct_info_to_bytes_equiv_concat:
  label:info_label
  -> context:serializable_bytes
  -> okm_len:okm_len_type
  -> Lemma (ensures (
...
let concat_context2_get_length
  (#cs:supported_cipherSuite)
  (ctx2:context2 #cs)
...
val lemma_construct_context2_to_bytes_equiv_concat:
  #cs:supported_cipherSuite
  -> c_r:c_id_bytes
  -> id_cred_r:id_cred_i_bytes
  -> th2:hash_out cs
...
let concat_context3_get_length
  (#cs:supported_cipherSuite)
  (ctx3:context3 #cs)
...
val construct_context3_to_bytes:
  ...
  -> ead3:option ead_bytes
  -> Tot (lbytes (
    concat_context3_get_length (construct_context3 id_cred_i th3 cred_i ead3)
  ))

val lemma_construct_context3_to_bytes_equiv_concat:
  #cs:supported_cipherSuite
  -> id_cred_i:id_cred_i_bytes
  -> th3:hash_out cs
  -> cred_i:cred_i_bytes
...
val lemma_extract_prk3e2m_equality:
  #cs:supported_cipherSuite
  -> salt1:hash_out cs
  -> salt2:hash_out cs
  -> ikm1:shared_secret cs
  -> ikm2:shared_secret cs
  -> Lemma (requires (
