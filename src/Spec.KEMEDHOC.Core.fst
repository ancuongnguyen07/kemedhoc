                ...
                let sig_or_mac2 = sig_or_mac2_raw in
                    ...
                    match prk3e2m_res with
                      ...
                      | Res prk3e2m -> (
                        ...
                        //   )
                        //   in
                        let is_valid_sig_or_mac2 = match remote_auth_material with
                          | Signature -> (
                        ...
                        if (not (is_valid_sig_or_mac2)) then Fail IntegrityCheckFailed
                          ...
                          if (unequal_lbytes_eq cred_r cred_i
                            || not (unequal_lbytes_eq #_ #(length is.remote_id_cred) cred_r is.remote_id_cred))
...
let initiator_send_msg3 #cs is hs ptx2 
      ...
      let prk4e3m_res:eresult (hash_out cs)
        = match auth_material with
          ...
          | MAC -> (
            ...
            let ephe_stat_shared = dh i g_y in
              ...
              let ciphertext3 = encrypt_plaintext3 ptx3 th3 prk3e2m in
                  ...
                  let th4 = compute_th4 th3 ptx3 cred_i in
                      ...
                      assert(
                        ...
                        match (decrypt_ciphertext3 remote_auth_material msg3 th3 prk3e2m) with
                          | Fail DecryptionFailed -> True
                          | Res decrypted_c3 -> (
                            if (not (is_valid_ptx3_size cs remote_auth_material (length decrypted_c3)))
                            then False
                            else
                              (
                                let (id_cred_i, sig_or_mac3, ead3_op) = deserialize_ptx3_raw_bytes #cs #remote_auth_material decrypted_c3 in
...
let derive_prk3e2m (#cs:supported_cipherSuite)
  ...
  {(get_auth_material Responder hs.method == MAC) ==> Some? r})
  :eresult (hash_out cs)
  = let auth_material = get_auth_material Responder hs.method in
  match auth_material with
      | Signature -> Res prk2e
      | MAC -> (
        match (dh (Some?.v r) hs.g_x) with
          | None -> Fail InvalidECPoint
          | Some g_rx -> (
            // derive SALT3e2m
            let salt3e2m = expand_salt3e2m prk2e th2 in
...
let derive_sig2 (#cs:supported_cipherSuite)
  (entr: HACLRandom.entropy)
  (sk_r:ec_signature_priv_key cs)
  (mac2:lbytes (mac23_size cs Signature))
  (c2:context2 #cs)
  :option (ecdsa_signature)
    ...
    | Some (y_gy, entr1) -> (
      ...
      match (dh y g_x) with
        ...
        | Some g_xy -> (
          ...
          // derive PRK3e2m
          let prk3e2m_res:eresult (hash_out cs)
            = derive_prk3e2m #cs hs prk2e th2 (Some rs.static_dh.priv) in
          ...
          match prk3e2m_res with
            ...
            | Res prk3e2m -> (
              ...
              let sig_or_mac2_op:option (sig_or_mac23_bytes cs local_auth_material)
              ...
              let sig_or_mac3 = sig_or_mac3_raw in
                  ...
                  match prk4e3m_res with
                    ...
                    | Res prk4e3m -> (
                      ...
                      //   )
                      //   in
                      let is_valid_sig_or_mac3 = match remote_auth_material with
                        | Signature -> (
