...
#include "edhoc_test_vectors_kem.h"
}
#include "cantcoap.h"
...
/*comment this out if you do not want to use KEM-KEM authentication method*/
// #define KEM_KEM
...
static void config_parser(cli::Parser *parser) {
	parser->set_optional<bool>("q", "qs", false, "Post-quantum mode.");
	parser->set_required<uint8_t>("m", "method", "Authentication method, from 0 to 4");
}
...
#define ORIG
	...
	c_i.method = (enum method_type) auth_method;
	...
	// set up fields for KEM-KEM method in Initiator context
	c_i.pk_kem_i.ptr = (uint8_t *) test_vector_pk_kem_i;
	c_i.pk_kem_i.len = sizeof(test_vector_pk_kem_i);
	c_i.sk_kem_i.ptr = (uint8_t *) test_vector_sk_kem_i;
	c_i.sk_kem_i.len = sizeof(test_vector_sk_kem_i);
	...
	// set up fields for KEM-KEM method in other party context
	cred_r.pk_kem.ptr = (uint8_t *) test_vector_pk_kem_r;
	cred_r.pk_kem.len = sizeof(test_vector_pk_kem_r);
#endif
	...
	if (!pq_mode) {
		x_len = 32; // EC point on curve
		g_x_len = 32; // EC point on curve
	} else {
		x_len = 1632; // ML-KEM-512 secret key length
		g_x_len = 800; // ML-KEM-512 public key length
	}

	BYTE_ARRAY_NEW(X_random, x_len, x_len);
	BYTE_ARRAY_NEW(G_X_random, g_x_len, g_x_len);
	BYTE_ARRAY_NEW(k_auth_r, 32, 32) // shared secret from KEM
	...
	if (!pq_mode) { // ECDH
		...
		/*Set fixed X and G_X for reproducible testing*/
		X_random.ptr = c_i.x.ptr;
		X_random.len = c_i.x.len;
		G_X_random.ptr = c_i.g_x.ptr;
		G_X_random.len = c_i.g_x.len;
		
		PRINT_ARRAY("secret ephemeral DH key (X)", X_random.ptr, X_random.len);
		PRINT_ARRAY("public ephemeral DH key (G_X)", G_X_random.ptr, G_X_random.len);
	} else { // PQC
		...
		if (c_i.method == INITIATOR_KEM_RESPONDER_KEM) {
