...
bool decode_repeated_map3_x5bag(
...
bool decode_repeated_map3_x5chain(
...
bool decode_ptxt3(
		zcbor_state_t *state, struct ptxt3 *result)
{
	zcbor_log("%s\r\n", __func__);
	bool int_res;

	bool tmp_result = (((((zcbor_union_start_code(state) && (int_res = ((((decode_map3(state, (&(*result).ptxt3_ID_CRED_I_map3_m)))) && (((*result).ptxt3_ID_CRED_I_choice = ptxt3_ID_CRED_I_map3_m_c), true))
	|| (zcbor_union_elem_code(state) && (((zcbor_bstr_decode(state, (&(*result).ptxt3_ID_CRED_I_bstr)))) && (((*result).ptxt3_ID_CRED_I_choice = ptxt3_ID_CRED_I_bstr_c), true)))
	|| (((zcbor_int32_decode(state, (&(*result).ptxt3_ID_CRED_I_int)))) && (((*result).ptxt3_ID_CRED_I_choice = ptxt3_ID_CRED_I_int_c), true))), zcbor_union_end_code(state), int_res)))
	&& ((zcbor_bstr_decode(state, (&(*result).ptxt3_SGN_or_MAC_3))))
	&& ((*result).ptxt3_EAD_3_present = ((zcbor_bstr_decode(state, (&(*result).ptxt3_EAD_3)))), 1))));
		...
		size_t *payload_len_out)
{
	...
	return zcbor_entry_function(payload, payload_len, (void *)result, payload_len_out, states,
		(zcbor_decoder_t *)decode_ptxt3, sizeof(states) / sizeof(zcbor_state_t), 3);
}
