...
{
	if (in1->len != in2->len || out->len != in1->len) {
		return xor_error;
	}
			...
			struct byte_array *key, struct byte_array *iv)
{
	switch (ctxt) {
	case CIPHERTEXT1:
		// only for KEM-KEM method
		// assume prk = k_auth_r, at this moment, just need to take the first
		// 16 bytes of k_auth_r just for experimenting
		// a mature design will come later
		key->ptr = prk->ptr;
		key->len = 16;

		// take the first 13 bytes of k_auth_r as IV
		// a mature design will come later
		iv->ptr = prk->ptr;
		iv->len = 13;
		break;
	...
	struct byte_array *ciphertext, struct byte_array *plaintext)
{
	...
	if (ctxt == CIPHERTEXT4 && plaintext->len != 0) {
		...
		ead->len = 0;
		PRINT_MSG("No EAD_4\n");
	} else if (ctxt == CIPHERTEXT1) {
		// KEM-KEM method
		TRY(plaintext1_split(plaintext, id_cred));
		PRINT_ARRAY("ID_CRED", id_cred->ptr, id_cred->len);
	} else {
		if (ctxt == CIPHERTEXT2) {
			TRY(plaintext2_split(plaintext, c_r, id_cred, sig_or_mac,
					    ead));
			PRINT_ARRAY("C_R (raw)", c_r->ptr, c_r->len);
		} else {
			TRY(plaintext3_split(plaintext, id_cred,
					    sig_or_mac, ead));
		}
		PRINT_ARRAY("ID_CRED", id_cred->ptr, id_cred->len);
		if (sig_or_mac->len) {
			...
			struct byte_array *plaintext)
{
	...
	// if (ctxt != CIPHERTEXT2 && ctxt != CIPHERTEXT1) {
	// 	/*Associated data*/
	...
	PRINT_ARRAY("ciphertext_1/2/3/4", ciphertext->ptr, ciphertext->len);
	return ok;
}
