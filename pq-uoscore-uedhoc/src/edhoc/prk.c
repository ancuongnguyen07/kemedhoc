...
enum err prk_derive(enum method_type method, bool static_dh_auth, struct suite suite, uint8_t label,
		    struct byte_array *context, const struct byte_array *prk_in,
		    const struct byte_array *stat_pk, const struct byte_array *stat_sk,
			const struct byte_array *k_auth, uint8_t *prk_out)
{
	// prk_in could be prk_2e/prk_3e2m/prk_4e3m
	if (method == INITIATOR_KEM_RESPONDER_KEM) {
		// KEM-KEM method, a non-standardized authentication method
		...
		// derives prk. k_auth could be k_auth_i/k_auth_r
		TRY(hkdf_extract(suite.edhoc_hash, &salt, k_auth, prk_out));
		return ok;
	}

	if (static_dh_auth) { // if a static DH key is used
		...
		// if a signature key is used then there is no key derivation happening
