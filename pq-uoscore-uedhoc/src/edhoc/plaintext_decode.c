...
enum err plaintext1_split(struct byte_array *ptxt,
				struct byte_array *id_cred_i) {
	size_t decode_len = 0;
	struct ptxt1 p;

	TRY_EXPECT(cbor_decode_ptxt1(ptxt->ptr, ptxt->len, &p, &decode_len), 0);

	/*ID_CRED_I*/
	if (p.ptxt1_ID_CRED_I_choice == ptxt1_ID_CRED_I_map1_m_c) {
		if (p.ptxt1_ID_CRED_I_map1_m.map3_x5chain_present) {
			PRINT_MSG("ID_CRED_I is x5chain\n");
			TRY(id_cred_x_encode(
				x5chain, 0,
				p.ptxt1_ID_CRED_I_map1_m.map3_x5chain
					.map3_x5chain.value,
				(uint32_t)p.ptxt1_ID_CRED_I_map1_m.map3_x5chain
					.map3_x5chain.len,
				id_cred_i));
		}
		if (p.ptxt1_ID_CRED_I_map1_m.map3_x5t_present) {
			PRINT_MSG("ID_CRED_I is x5t\n");
			TRY(id_cred_x_encode(x5t,
					     p.ptxt1_ID_CRED_I_map1_m.map3_x5t
						     .map3_x5t_alg_int,
					     p.ptxt1_ID_CRED_I_map1_m.map3_x5t
						     .map3_x5t_hash.value,
					     (uint32_t)p.ptxt1_ID_CRED_I_map1_m
		...
		if (p.ptxt1_ID_CRED_I_choice == ptxt1_ID_CRED_I_map1_m_c) {
			TRY(id_cred_x_encode(
				kid, 0, p.ptxt1_ID_CRED_I_bstr.value,
				(uint32_t)p.ptxt1_ID_CRED_I_bstr.len,
				id_cred_i));

		} else {
			int _kid = p.ptxt1_ID_CRED_I_int;
			TRY(id_cred_x_encode(kid, 0, &_kid, 1, id_cred_i));
		}
	}
}

enum err plaintext2_split(struct byte_array *ptxt,
...
enum err plaintext3_split(struct byte_array *ptxt,
				 struct byte_array *id_cred_i,
				 struct byte_array *sign_or_mac,
				 struct byte_array *ead)
{
